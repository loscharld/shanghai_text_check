...
def Comparison_two(fr1, fr2):
    ...
    word_list1=extract1(line1).split(' ')
    ...
    word_list2=extract1(line2).split(' ')
    return word_list1, word_list2

def Comparison_two1(fr1, fr2):
    punc = './ <>_ - - = ", 。，？！“”：‘’@#￥% … &×（）——+【】{};；● &～| \s:'
    stoplist = {}.fromkeys([line.rstrip() for line in
                            codecs.open(r"中文停用词库.txt", 'r', 'utf-8')])  # 建立字典，词库中的字符为键
    # with open(fr1, 'r', encoding='utf-8') as f:
    ...
    line1 = re.sub(r"[{}]+".format(punc), "", string)  # 将文本中标点符号去掉
    # word_list1 = [word.word for word in pesg.cut(line1) if word.flag[0] not in ['w', 'x', 'u']]
    word_list1=extract2(line1).split(' ')
    # with open(fr2, 'r', encoding='utf-8') as f:
    #     list2 = f.read()
    string = ''
    X, Y = ['\u4e00', '\u9fa5']
    ...
    line2 = re.sub(r"[{}]+".format(punc), "", string)
    # word_list2 = [word.word for word in pesg.cut(line2) if word.flag[0] not in ['w', 'x', 'u']]
    word_list2=extract2(line2).split(' ')
    return word_list1, word_list2
...
def extract2(line):
    ...
    guanjian=tfidf(str1_rv_stop_word_fenci,topK=600)
    guanjian_result=''
    lianshi=[]
    for each in str1_rv_stop_word_fenci.split(' '):
        if each in guanjian:
...
def duanluo_sim(model,doc,content,n1):
    tfidf = analyse.extract_tags
    _,doc,_=chaifen_doc(doc)
    _,content,_=chaifen_doc(content)
    duoluo_all=[]
    for d1 in doc:
        guanjian1=tfidf(d1,topK=200)
        for d2 in content:
            duoluo_dict = {}
            guanjian2=tfidf(d2,topK=200)
            # 转成句子向量
...
# Create server
with ThreadXMLRPCServer(("127.0.0.1", 5009), requestHandler=RequestHandler) as server:
    server.register_introspection_functions()
    ...
    def multiple_sim(files,n1):
        ...
        doc_sim_all.sort_values('sim_score', ascending=False, inplace=True)
        doc_sim_all.to_excel('data/多篇文档相似度.xlsx', index=None, encoding='utf-8')
        # if not doc_sim_all:
        #     return None
        # return doc_sim_all
