...
def guolv(juzi):
    punc = './ <>_ - - = ", 。，？！“”：‘’@#￥% … &×（）——+【】{};；● &～| \s:'
    text1 = re.sub(r'[^\w]+', '', juzi)
    line1 = re.sub(r"[{}]+".format(punc), "", text1)# 将文本中的特殊字符去掉
    return line1
...
def sent2vec(model, words):
    ...
    vect_sum=vect / np.sqrt((vect ** 2).sum())
    return vect_sum
...
def duanluo_sim(model,doc,content,n1):
    tfidf = analyse.extract_tags
    len_ju=ju_chaifen(doc)
    if len(len_ju)>=10:
        doc=chaifen_doc1(doc,10)
        content=chaifen_doc1(content,10)
    elif len(len_ju)==1:
        doc= chaifen_doc(doc,len(len_ju))
        content= chaifen_doc(content,len(len_ju))
    else:
        doc = chaifen_doc(doc, len(len_ju))
        content = chaifen_doc(content, len(len_ju)-1)
    duoluo_all=[]
    for d1 in doc:
        dd1=guolv(d1)
        guanjian1=tfidf(dd1,topK=200)
        for d2 in content:
            dd2=guolv(d2)
            duoluo_dict = {}
            guanjian2=tfidf(dd2,topK=200)
            # 转成句子向量
            try:
                vect1 = sent2vec(model, guanjian1)
                vect2 = sent2vec(model, guanjian2)
                cos = similarity(vect1, vect2)
                two_sim = round(cos, 3)
            except:
                continue
            if float(two_sim) >n1:
                duoluo_dict['Original_para']=d1
                duoluo_dict['Contrast_para']=d2
                duoluo_dict['Para_sim']=two_sim
                duoluo_all.append(duoluo_dict)
    duoluo_all=sorted(duoluo_all, key=lambda s: s['Para_sim'], reverse=True)
...
def result_sim(doc):
    ...
    for line in ceshi_list:
        ...
        for count, sim in sims:
            ...
            try:
                duanluo_all=[duanluo_sim(model,doc,content,0.85)]
            except:
                duanluo_all=[]
            doc2sim['Para_comparison']=duanluo_all
...
def func(listTemp, n):
    for i in range(0, len(listTemp)):
        fenju=listTemp[i:i + n]
        yield fenju

def func1(listTemp, n):
    for i in range(0, len(listTemp),10):
        fenju=listTemp[i:i + n]
        yield fenju


def chaifen_doc(doc,n1):
    punc = './ <>_ - - = ", 。，？！“”：‘’@#￥% … &×（）——+【】{};；● &～| \s:《》'
...
def chaifen_doc1(doc,n1):
    list1 = re.split(r"([。!！])", doc)
    z_list1 = ["".join(i) for i in zip(list1[0::2], list1[1::2])]  # 将文章分成每一句为一个元素存放于列表中
    exam = func1(z_list1, n1)
    for i in exam:
        if len(i)<n1:
            continue
        t1 = "".join(i)
...
with ThreadXMLRPCServer(("127.0.0.1", 5002), requestHandler=RequestHandler) as server:
    ...
    def multiple_sim(files,n1):
        ...
        for i in range(len(pathes)):
            path=pathes[i].replace('/','\\')
            if pathes[i].endswith('.json'):
                top=json.load(open(path))
            else:
                # top=open(path,'r',encoding='utf-8').read()
                top=''.join(list(open(path, 'r', encoding='utf-8').readlines()))
            try:
                if '中文摘要' in top:
            ...
            for j in range(i+1,len(pathes)):
                path1=pathes[j].replace('/','\\')
                if j!=i:
                    if pathes[j].endswith('.json'):
                        tail = json.load(open(path1))
                    else:
                        # tail=open(path1,'r',encoding='utf-8').read()
                        tail = ''.join(list(open(path1, 'r', encoding='utf-8').readlines()))
                    try:
                        if '中文摘要' in tail:
                    ...
                    if top_zy and tail_zy:
                        try:
                            ...
                            if two_sim1 >= 0.85:
                                ...
                                try:
                                    ...
                                    st1, st2 = Comparison_two2(top1, tail1)
                                    # 转成句子向量
                                    vect1 = sent2vec(model, st1)
                                    vect2 = sent2vec(model, st2)
                                    cos = similarity(vect1, vect2)
                        ...
                        try:
                            ...
                            st1, st2 = Comparison_two(top_zy, tail1)
                            # 转成句子向量
                            vect1 = sent2vec(model, st1)
                            vect2 = sent2vec(model, st2)
                            cos = similarity(vect1, vect2)
                        ...
                        try:
                            ...
                            top1 = top_zdzy + top1
                            st1, st2 = Comparison_two(top1, tail_zy)
                            # 转成句子向量
                            vect1 = sent2vec(model, st1)
                            vect2 = sent2vec(model, st2)
                            cos = similarity(vect1, vect2)
                        ...
                        try:
                            ...
                            top1 = top_zdzy + top1
                            tail1 = tail_zdzy + tail1
                            st1, st2 = Comparison_two(top1, tail1)
                            # 转成句子向量
                            vect1 = sent2vec(model, st1)
                            vect2 = sent2vec(model, st2)
                            cos = similarity(vect1, vect2)
                    ...
                    if float(sim_result) < n1:
                        ...
                        # print("相似度：{:.4f}".format(cos))
                        doc_n = 'sim_score'
                        doc_sim['Stxt']=pathes[i]
                        doc_sim['Ctxt']=pathes[j]
                        doc_sim[doc_n] = str(sim_result)
                        doc_sim_all.append(doc_sim)
        if not doc_sim_all:
            return None
        return doc_sim_all
    ...
    def hash_sim_(duibi_doc, doc, n1=0.4):
        ...
        # 多篇相似度
    def multiple_sim_(path,n1=0.6):
        path=eval(path)
        n1=float(n1)
        multi_sim1 = multiple_sim(path,n1)
