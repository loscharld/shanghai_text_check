...
# Create server
with ThreadXMLRPCServer(("127.0.0.1", 5003), requestHandler=RequestHandler) as server:
    server.register_introspection_functions()
    def multiple_sim(path, n1=0.9):
        print("load model")
        Dir=os.listdir(path)
        pathes=[path+'/'+pt for pt in Dir]
        doc_sim_all =[]
        for i in range(len(pathes)):
            if pathes[i].endswith('.json'):
                top=json.load(pathes[i])
            else:
                top=open(pathes[i],'r',encoding='utf-8').read()
            for j in range(i+1,len(pathes)):
                if j!=i:
                    if pathes[j].endswith('.json'):
                        tail = json.load(pathes[i])
                    else:
                        tail=open(pathes[j],'r',encoding='utf-8').read()
                    doc_sim={}
                    model = Doc2Vec.load('model_doc/model_sim')
                    st1, st2 =Comparison_two(top, tail)
                    # 转成句子向量
    ...
    #多篇相似度
    def multiple_sim_(path,n1=0.5):
        wb_sim1 = multiple_sim(path,n1=0.9)
